...
import { commissionModels } from "@/lib/data/recruitment";
    ...
    <section className="py-28 bg-[#f7f7f7] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 max-w-3xl mx-auto gap-px bg-gray-200">
          {commissionModels.map(({ name, structure, fee, best, features, featured }, i) => (
            <motion.div
              key={name}
            ...
            >
              ...
              <ul className="space-y-3 mb-10">
                {features.map((f, j) => (
                  <motion.li
                    key={f}
                    className={`flex items-start gap-3 text-sm font-light ${featured ? "text-gray-300" : "text-gray-600"}`}
              ...
              <Link href="#apply" className={`block text-center text-sm font-light tracking-widest uppercase py-4 transition-colors ${featured ? "bg-[#c8a96e] text-black hover:bg-white" : "border border-gray-200 text-gray-700 hover:border-black hover:text-black"}`}>
                Apply Now
              </Link>
            </motion.div>
          ))}
